...
COLORREF GetTransparentColor(HBITMAP hBitmap) {
    // Получаем цвет верхнего левого пикселя как прозрачный
    HDC hdc = CreateCompatibleDC(NULL);
    SelectObject(hdc, hBitmap);

    COLORREF transparentColor = GetPixel(hdc, 0, 0); // Цвет верхнего левого пикселя
    DeleteDC(hdc);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    HWND hWnd = CreateWindow(wc.lpszClassName, _T("Практическая работа 2"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 800, 600, NULL, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    static COLORREF transparentColor = RGB(255, 255, 255); // Прозрачный цвет по умолчанию

    switch (message) {
    case WM_CREATE:
        hButton = CreateWindow(_T("BUTTON"), _T("Выбор файла"), WS_VISIBLE | WS_CHILD, 10, 10, 120, 30, hWnd, (HMENU)1, GetModuleHandle(NULL), NULL);
        break;
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == 1 && SelectFile(hWnd)) {
            ...
            if (hBitmap) {
                transparentColor = GetTransparentColor(hBitmap); // Устанавливаем прозрачный цвет
            }
            else {
    ...
    case WM_PAINT: {
        ...
        if (hBitmap) {
            ...
            int newWidth = clientRect.right / 2; // Ширина изображения после масштабирования
            int newHeight = clientRect.bottom / 2;  // Высота изображения после масштабирования
            
            // Координаты для размещения справа снизу
            int x = clientRect.right - newWidth;   // Расчет по ширине окна
            int y = clientRect.bottom - newHeight; // Расчет по высоте окна
            ...
            // Рисуем изображение с учетом прозрачного цвета
